# Flushnet AI Runner

Run browser, terminal, and AI tasks across your machines and remote nodes from one place.

The fastest way to connect a machine, launch a task, watch execution, and get the result from one operator layer.

Use your own machines or secure sandboxes. Run tasks in parallel. Control everything from the dashboard, API, or AI assistant.

Logged in as: marchese.luciano@gmail.com · Owner ID: user_931d4e7f · Plan: pending

[Open dashboard](https://www.flushnet.net/api/dashboard/) · [Open entitlement admin](https://www.flushnet.net/api/dashboard/) · [Install agent](https://www.flushnet.net/api/installer.php) · [View docs](https://www.flushnet.net/api/docs.php) · [Open AI Runner Chat](https://chatgpt.com/g/g-69c4cd2538d0819185bb5e3a2dde197d-ai-runner)

## Start with the AI Runner Chat in ChatGPT

For the easiest setup, begin in the custom AI Runner chat, then link it to your Flushnet account with an access code from the dashboard.

1. Open [AI Runner Chat](https://chatgpt.com/g/g-69c4cd2538d0819185bb5e3a2dde197d-ai-runner) in ChatGPT.
2. Log in or create your Flushnet account.
3. Open [Dashboard](https://www.flushnet.net/api/dashboard/) and then Generate Access Code.
4. Paste the access code into the AI Runner chat when asked.
5. Install your local runtime or use sandbox mode, then ask the chat to list nodes and run your first task.

[Open AI Runner Chat](https://chatgpt.com/g/g-69c4cd2538d0819185bb5e3a2dde197d-ai-runner) · [Generate Access Code](https://www.flushnet.net/api/dashboard/) · [See onboarding examples](https://www.flushnet.net/api/docs.php)

## AI that uses your computer & browser

AI Runner turns requests into real actions across desktop, browser, mobile, and connected devices.

Use one AI operator layer to organize files, automate workflows, fill forms, test apps, run programs, manage email and calendars, compare products, create reports, place calls, chat on Telegram or WhatsApp, and control IoT devices.

Built for practical execution: browser automation, terminal tasks, multimodal AI, mobile camera and voice input, call center and conferencing flows, sandbox and virtual machines, RAG memory, third-party app integrations, and support for ChatGPT, Claude, Gemini, Ollama, Hugging Face, and llama.cpp.

## Some of the tasks you can ask AI Runner to do

- Organize my computer files
- Automate and run this workflow
- Fill in this online form for me
- Build and test a mobile app
- Run a program on my computer
- Check the last emails from my boss and write a reply with the latest work updates
- Test and debug my app
- Write and post my new event to my Facebook page
- Create a photo gallery of my personal photos
- Book a restaurant reservation
- Search and compare prices for this product
- Search in “My Documents” for statements and write a full report in PDF format
- Schedule an appointment on my calendar
- Dial this telephone number
- Read my Telegram and WhatsApp messages
- Use the mobile phone app or desktop app with live audio voice to open and write an MS Word document
- Use the mobile camera app to send photos and videos to this chat
- Use the mobile app to show my current AI Runner tasks
- Chat with AI Runner from Telegram and WhatsApp in text and audio chat
- Enable all my home IoT smart devices such as sockets, cameras, and robots

## More features include

- Windows and Mac installers
- Support for OpenAI ChatGPT models 5.4, 5.3, and 5.2, Anthropic Claude, and Google Gemini
- Full integration with OpenClaw, open-source, and third-party AI agents
- Integration with Ollama, Hugging Face, and llama.cpp
- Full multimodal support and custom multimodal AI models
- Task and workflow optimization
- AI multi-channel support including IoT, WhatsApp, Telegram audio, phone, call center, video call, multi-user conference, and more
- Third-party and open-source app integration
- Sandbox and virtual machine mode
- RAG and persistent long-ter
